...
void handleNoteOff(byte channel, byte pitch, byte velocity) {
  if (noteIsInC(int(pitch))) {
    /*
     * Handling note off from previous keySig ----------------------------------------------
     */
    if (noteFreeze[int(pitch)] == true) {
      noteFreeze[int(pitch)] = false; // this note will be handled correctly
    ...
    /*
     * Handling normal note off (not a note from before key change) -------------------------------------
     */
    else {
      if (keySig == 1) {
        if ((int(pitch) - 4) % 12 == 0 || (int(pitch) - 9) % 12 == 0 || (int(pitch) - 11) % 12 == 0) {
